...
class Developer(commands.Cog, name="Developer"):
    ...
    async def create(self, ctx, name):
        ...
        if MAX_CHANNELS_CHECK and category is not None and len(category.channels) >= MAX_CHANNELS:
            embed = discord.Embed(title="Slow down there cowboy!",
                                  description=f"You can only have {MAX_CHANNELS} active projects at a time!",
        ...
        # Checks to see if the user already has a project with the same name
            ...
            for scan in category.channels:
                if scan.name == name:
                    embed = discord.Embed(title="Whoops!", description="You already have a project with that name!",
                                          color=discord.Color.red())
                    await ctx.send(embed=embed)
                    valid = False
        ...
        if PROFANITY_CHECK and name.lower in BLACKLIST_WORDS:
            embed = discord.Embed(title="Woah there!",
                                  description="Your project contained profanity!\n\n**Reminder:**\n- No projects with vulgar names\n- Projects must be child friendly",
                                  color=discord.Color.red())
            await ctx.send(embed=embed)
            valid = False
        ...
        if valid is True:
            ...
            embed = discord.Embed(title="Success!", description="Your project is ready to go!",
    ...
    async def finish(self, ctx, project):
        ...
        if category is None:
            embed = discord.Embed(title="Whoops!", description="You don't have any active projects!",
                                  color=discord.Color.red())
            await ctx.send(embed=embed)
        else:
            for scan in category.channels:
                if scan.name == project:
                    ...
                    if CONFIRMATION_PROMPT is True:
                        ...
                        embed = discord.Embed(title="Confirmation",
                                              description=f"Please type `{CONFIRMATION_WORD}` to finish the project.\n\n**What this does:**\n\t- Deletes the project in your category\n\t- Adds project to completion board",
                                              color=discord.Color.blue())
                        await ctx.send(embed=embed)
                    ...
                    # Prompts user with countdown if enabled
                        ...
                        await ctx.send(embed=embed)
                        embed = discord.Embed(title="Warning!",
                                              description=f"This channel will be deleted in {COUNTDOWN_TIME} seconds!",
                                              color=discord.Color.red())
                        await channel.send(embed=embed)
                        ...
                        embed = discord.Embed(title="Warning!",
                                              description="This channel will be deleted in 15 seconds!",
                                              color=discord.Color.red())
                        await channel.send(embed=embed)
                    ...
                    if confirmation_passed is True:
                        embed = discord.Embed(title=":confetti_ball: Project complete! :confetti_ball:",
                                              description="Congratulations on finishing your project!\n\n**Now what?**\n\t- Upload to the marketplace\n\t- Share it with others\n\t- Get engaged with your audience!",
                                              color=discord.Color.blue())
                        await ctx.send(embed=embed)
                        category = get(ctx.guild.categories, name=username)
    ...
    async def toggle(self, ctx):
        """Opt-in or opt-out of receiving invites. This feature is still in development"""
        with open("opt-out.json") as f:
            data = json.load(f)
        i = 0
        for user in data:
            if user["user_id"] == ctx.author.id:
                del data[i]
                with open("opt-out.json", "w") as f:
